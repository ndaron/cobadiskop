...
class Poll_Maker_Ays_Public {
	...
	public function ays_poll_generate_html( $poll_id, $echo = true, $width = -1 ) {
        ...
        #".$this_poll_id.".box-apm .apm-choosing .ays-poll-answer-container-list{
			display: flex;
			width: 100%;
        }";
						...
						switch ( $poll['type'] ) {
							case 'choosing':
								...
								foreach ( $poll['answers'] as $index => $answer ) {
									if ($answer['user_added'] == 1 && $answer['show_user_added'] ==  0) {
										...
										if ($answer['show_user_added'] ==  0){
											...
											<label for='radio-$index-$this_poll_id' class='ays_label_poll ".$answers_sound_class." ".$redirect_after_submit." ".$disable_answer_hover." ays_label_font_size ".$answer_icon_class." ".$poll_class_for_answer_label." ".$pol_answer_view_type_label_cont."' answers-url='".$answer['redirect']."'>".$poll_answer_image_show." <div><span class='".$pol_answer_view_type_text_show."'>"
								...
								if ($poll_allow_answer && $with_vote) {
									...
									";
									// $poll_add_answer_for_choosing = true;
								}
								break;
							case 'voting':
